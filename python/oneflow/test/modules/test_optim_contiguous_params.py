...
def compare_with_numpy_sgd(
    ...
    def train_by_contiguous_param():
        x_tensor_list = []
        for i in range(tensor_num):
            x_tensor_list.append(
                flow.Tensor(init_value_seq[i], device=flow.device(device))
            )
        x_c = ContiguousParams(x_tensor_list)

        sgd = flow.optim.SGD(
            [
                {
                    "params": [x_c.contiguous()],
                    "lr": learning_rate,
        ...
        def train_one_iter(grad):
            ...
            loss = flow.sum([xi * grad_tensor for xi in x_c.original()])
            loss.backward()
            sgd.clip_grad()
            sgd.step()
            sgd.zero_grad()
