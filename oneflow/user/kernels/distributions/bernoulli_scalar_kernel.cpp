...
// NOTE(Liang Depeng): The implementation of BernoulliScalarCpuKerenl is modified from
//                    https://github.com/pytorch/pytorch/blob/master/aten/src/ATen/native/cpu/DistributionTemplates.h
namespace oneflow {

template<typename T>
class BernoulliScalarCpuKerenl final : public user_op::OpKernel {
 public:
  BernoulliScalarCpuKerenl() = default;
  ~BernoulliScalarCpuKerenl() override = default;
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx, user_op::OpKernelState* state,
    ...
    for (int64_t i = 0; i < elem_cnt; ++i) {
      ...
      out_dptr[i] = static_cast<T>(random_val < p);
    }
  }
  bool AlwaysComputeWhenAllOutputsEmpty() const override { return false; }
};

#define REGISTER_BERNOULLI_SCALAR_CPU_KERNEL(out_dtype)               \
  REGISTER_USER_KERNEL("bernoulli_scalar")                            \
      .SetCreateFn<BernoulliScalarCpuKerenl<out_dtype>>()             \
      .SetIsMatchedHob((user_op::HobDeviceType() == DeviceType::kCPU) \
                       && (user_op::HobDataType("out", 0) == GetDataType<out_dtype>::value));

REGISTER_BERNOULLI_SCALAR_CPU_KERNEL(float)
REGISTER_BERNOULLI_SCALAR_CPU_KERNEL(double)
